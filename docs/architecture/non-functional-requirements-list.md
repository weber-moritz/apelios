...
- Reliability & Fault Tolerance (Stability): * The system must not crash if a physical input device is disconnected mid-operation. The Router must simply drop the missing messages until the device returns.
...
- Extensibility & Maintainability (Modular): * Adding support for a new physical input device must only require adding a new Input Adapter script, with zero changes to the Router or Fixture Layer.
